...
class racing_model extends SQLData{
	...
	$this->query($sql);
	$sql = "INSERT INTO tbl_rank_user (user_id,score)
	(
	SELECT user_id, sum(score) as score  FROM ".RedRushDB.".tbl_exp_point GROUP BY user_id ORDER BY score DESC
	);";
	$result = $this->query($sql);
	}else return false;
	}
	...
	function get_merchandise_item(){
		$sql = "SELECT * FROM ".RedRushDB.".rr_merchandise WHERE n_status='1' ";
		return $this->fetch($sql,1);
	}
	
	function get_merchandise_item_by_id($merchandise_id=NULL){
		$sql = "SELECT * FROM ".RedRushDB.".rr_merchandise WHERE id = ".intval($merchandise_id)." AND n_status='1' LIMIT 1" ;
		return $this->fetch($sql);
	}
	...
	function get_own_merchandise($user_id,$merchandise_id){
		$sql = "SELECT count(id) as total FROM ".RedRushDB.".rr_purchase_merchandise WHERE user_id=".$user_id." AND 	merchandise_id =".$merchandise_id." AND n_status !='2' ";
		$data =  $this->fetch($sql);
		return $data['total'];
	}
	...
	function purchase_merchandise($user_id,$merchandise_id,$data_personal){
		...
		(purchase_merchandise_id, user_id, merchandise_id, purchase_date, prize) 
		VALUES 
		('".$purchased_id."',".$user_id.", ".$merchandise_id.", NOW(), ".$merchandisePrice['prize'].");
		";
		...
		} else return array('message'=>'There is an error with the database.'.$sql,'result'=>0);
		} else return array('message'=>'You not have enough points to redeem','result'=>0,'total'=>$userPoint);
	...
	WHERE 
	user1_id = ".$user_id." AND 
	user2_id=".$opponent_id." AND
	...
	$sql ="
		INSERT INTO ".GameDB.".racing_level 
		(user_id,level) 
		VALUES 
		(".$user_id.",".$new_level.")		
		";
	...
	$sql = "SELECT *,ultimate_id as id FROM ".RedRushDB.".rr_ultimate_car WHERE level=".$level." LIMIT 1";
	return $this->fetch($sql);
