...
class mobile_model extends SQLData{
	...
	$nextPage = $page+1;
	$page = ($page*$total)-$total;
	$opponent = $this->get_opponent($user_id,null, $player['level'],$page,$total,false);
	...
	(id,email,survey_date,surveyor,data_gather_date_time,data_gather_date_ts) 
	VALUES 
	(".intval($value->id).",'".$value->email."','".$value->survey_date."','".$value->surveyor."','".$data_gather_date_time."','".$data_gather_date_ts."')
	";
	$this->query($sql);
	...
	(email,n_status,verified,mobile_type) 
	VALUES 
	('".$value->email."',1,'1','2')
	";
	$this->query($sql);
	...
	$sql ="
	SELECT id,user_id,game_id,survey_date 
	FROM ".ReportDB.".tbl_ipad_data_game 
	WHERE user_id ='{$user_data['user_id']}'
	AND n_status=0 
	GROUP BY game_id, user_id, date_format(survey_date,'%Y-%m-%d') 
	...
	return array('total data'=>$debug_data);
	...
	//complete the part level
		...
		foreach($partData as $part){
			...
			$sql ="INSERT IGNORE INTO 
					...
					VALUES
					({$user_id},{$part['id']},{$part['price']},NOW(),'".time()."')
			";	
			$this->query($sql);
