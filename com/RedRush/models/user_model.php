...
class user_model extends SQLData{
	...
	if($user_id==999) $name = 'Ultimate Car Level 3';
	if($user_id==9999) $name = 'Ultimate Car Level 5';
	...
	function note_play_mini_game($user_id,$game_id){
		...
		if($game_id==3) $note = $this->after_findobject_game($player);
		if($game_id==4) $note = $this->after_segway_game($player);
		return $note ;
	}
	...
	function get_user_notification($user_id,$total){
		...
		$sql = "(SELECT  a.user2_id as user_id, a.user1_id as action_value, concat(a.circuit_id,'_',a.winner) as info, a.date_time as date_time, 4 as action_id, a.report_id as report_id, a.has_read as has_read FROM ".GameDB.".racing_history a WHERE user2_id=".$user_id.")
				UNION
				(SELECT b.user_id as user_id, b.merchandise_id as action_value, b.prize as info , b.purchase_date as date_time, 6 as action_id, 0 as report_id, b.has_read as has_read FROM ".RedRushDB.".rr_purchase_merchandise b WHERE b.user_id=".$user_id.")
		...
		if($notification['action_id'] == 4 ) $note[] =  array('small_img'=>$small_img['small_img'],'message'=>$this->get_user_race_notification($notification),'date_time'=>$date->datediff($notification['date_time']),'report_id'=>$notification['report_id'] ,'has_read'=>$notification['has_read']); //racing
		if($notification['action_id'] == 6 ) $note[] =  array('small_img'=>$small_img['small_img'],'message'=>$this->get_user_merchandise_notification($notification),'date_time'=>$date->datediff($notification['date_time']),'report_id'=>NULL ,'has_read'=>$notification['has_read']);  //purchase merchandise
	...
	function send_user_notification($user_id,$opponent_id,$winner,$report,$circuit){
		...
		$token = sha1($opponent_id.sha1(date('YmdH')));
...
<!-- End Save for Web Slices -->
	...
	WHERE (b.nickname like '%".$search."%' OR b.name like '%".$search."%' OR b.username like '%".$search."%' OR b.last_name like '%".$search."%') AND b.n_status=1 ";
	$data = $this->fetch($sql,1);
	return $data;
	}	
